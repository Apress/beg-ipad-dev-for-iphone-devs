...
- (void)deselectAllToolButtons {
	...
	[dotButton setImage:[UIImage imageNamed:@"button_cdots.png"]];
}
...
- (void)createDocument {
  [self saveCurrentToFile:[FileList sharedInstance].currentFile];
  [[FileList sharedInstance] createAndSelectNewUntitled];
...
- (void)renameCurrentDocument {
  ...
  controller.originalFilename = [FileList sharedInstance].currentFile;
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
  ...
  if (buttonIndex == 1) {
    [[FileList sharedInstance] deleteCurrentFile];
    [self loadFromFile:[FileList sharedInstance].currentFile];
    //dudelView.drawables = [NSMutableArray array];
    //[dudelView setNeedsDisplay];
  }
}
...
- (void)viewDidLoad {
  ...
  NSString *filename = [FileList sharedInstance].currentFile;
